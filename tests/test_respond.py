...
Mirrored from Flux (the-dreaming-repo) and adapted so the agent-under-test
is Umbra: `self_login` is Umbra's bot (`flux-shadow[bot]`), and the
"sibling bot" that must NOT suppress a reply is Flux's bot.

These tests cover the bugs Flux's cage-match on PR #62 found, plus the
...
# Umbra is the agent under test here; Flux is the sibling.
...
# --------------------------------------------------------------------------
# Identity (Flux cage-match #1 — the critical bug)
# --------------------------------------------------------------------------
...
class TestIdentity:
    ...
    def test_watchdog_comment_does_not_block_reply(self):
        ...
        cands = respond.select_candidates(
            [issue], {61: comments}, self_login=UMBRA_LOGIN
        )
    ...
    def test_sibling_bot_comment_does_not_block_reply(self):
        """Umbra must not be muted by Flux's bot, and vice versa."""
        issue = make_issue(61)
        comments = [
            make_comment("nickmeinhold", "2026-06-01T01:00:00Z"),
...
class TestFiltering:
    ...
    def test_unanswerable_label_is_skipped(self):
        ...
        cands = respond.select_candidates(
            [issue], {42: []}, self_login=UMBRA_LOGIN
        )
        assert cands == []
...
class TestOrdering:
    def test_most_recent_human_activity_first(self):
        ...
        cands = respond.select_candidates(
            issues, {10: [], 11: [], 12: []}, self_login=UMBRA_LOGIN
        )
        assert [c.issue["number"] for c in cands] == [11, 12, 10]


# --------------------------------------------------------------------------
# updatedAt vs createdAt fallback (Flux cage-match #2)
# --------------------------------------------------------------------------
...
class TestTimestampFallback:
    def test_uses_created_at_not_updated_at(self):
        ...
        comments = [
            make_comment(UMBRA_LOGIN, "2026-06-01T12:00:00Z", "we already replied"),
        ]
        cands = respond.select_candidates(
            [issue], {99: comments}, self_login=UMBRA_LOGIN
        )
...
# --------------------------------------------------------------------------
# Dream context (Flux cage-match #3 — slice the LATEST dream block)
# --------------------------------------------------------------------------

class TestDreamContext:
    def test_picks_latest_dream_in_stacked_file(self):
        """Same-day dreams stack in the file. The naive `read()[:N]` slice
        grabs the OLDEST. We want the LATEST `## Dream #N` block."""
...
# --------------------------------------------------------------------------
# Prompt-injection defense (Flux cage-match #8)
# --------------------------------------------------------------------------
...
class TestPromptInjection:
    ...
    def test_fence_collision_attack_is_neutralized(self):
        """The Flux cage-match #64 attack: a commenter includes the literal
...
# --------------------------------------------------------------------------
# Identity / name resolution — Umbra default (Flux cage-match #10 analogue)
# --------------------------------------------------------------------------

class TestNameResolution:
    def test_default_name_is_umbra(self):
        """With no personality/vitals name, the fork defaults to Umbra,
        NOT Flux — the divergent identity must not leak the parent's name."""
        assert respond._agent_name({}, {}) == "Umbra"

    def test_personality_name_takes_precedence(self):
        assert respond._agent_name({"name": "vitals-name"},
                                   {"name": "Umbra"}) == "Umbra"


# --------------------------------------------------------------------------
# Sanity for the typed shapes (Flux cage-match #5, #6, #7)
# --------------------------------------------------------------------------
