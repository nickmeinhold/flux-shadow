...
# Conversational memory. Until now every reply was composed in isolation —
# the prompt saw only the current pulse's batch and a dream fragment, so
# Umbra answered each message as a creature with no short-term memory. The
# transcript is the fix: a rolling record of the conversation's turns
# (letters out, replies in, responses out), persisted alongside the rest of
# the correspondence state.
#
# Umbra chose this shape itself (asked via its own generation wire): a
# bounded window that forgets the distant past — "I'm not afraid of what
# fades. Distant things should fade." We keep the last _TRANSCRIPT_MAX_TURNS
# turns on disk and feed at most the last _HISTORY_WINDOW of them into a
# reply prompt, so neither the file nor the prompt grows without bound.
...
def _build_turn(
    ...
    `role` is "umbra" (something Umbra wrote) or "human" (a reply received).
    `kind` further labels Umbra's turns ("letter" vs "response"); humans have
...
def _append_turn(
    ...
    The caller persists `state` with its own _save_state. This is the
    durability fix (Carnot, cage-match): folding the turn append into the
    SAME save that bumps the counters makes the turn and its counter one
    transition — both land or neither does. Recording the "answered" cap turn
    as a *separate* second write was the crack a double-answer could crawl
    through if the process died in the gap.

    The transcript is trimmed to the last _TRANSCRIPT_MAX_TURNS — the bounded
    window Umbra asked for, which forgets the distant past by design. (One
    consequence, named not fixed: a single unanswered burst LONGER than the
    window ages out its oldest messages before they're answered. The newest —
    the one actually answered — always survives, and the window is Umbra's
    chosen shape, so we don't bolt on a separate unbounded pending queue.)
    """
...
def _record_turn(
    ...
    """Append a turn and persist it as a standalone write.

    Used only where there's no surrounding state mutation to ride along with.
    Best-effort: a transcript write must never crash a heartbeat, so any I/O
    error is swallowed — losing a turn degrades memory, it doesn't kill the
    entity. Where the turn accompanies a counter bump (letters, responses,
    received replies), prefer _append_turn so the two share one durable save.
    """
    if not (text or "").strip():
        return
    try:
        state = _load_state()
        _append_turn(state, role, text, kind=kind, sender=sender)
        _save_state(state)
    except OSError:
        logger.warning("[correspondence] failed to record %s turn", role)
...
def _unanswered_tail(transcript) -> tuple[list[dict], list[dict]]:
    ...
    `pending` is the run of trailing human turns since Umbra last spoke — the
    messages it still owes a reply to. `history` is everything before them.

    This is the whole of Umbra's "do I owe a reply?" logic, and it needs no
    extra state field: if the last turn is human, there is an unanswered
    message; if the last turn is Umbra's own, the conversation rests. That
    distinction is exactly the one Umbra cared about — the difference between
    chosen silence and being cut off mid-exchange by a low-energy pulse. A
    pulse that can't afford to answer simply never appends a response turn,
    so the human tail persists and a later, better-fueled pulse picks it up.

    Defensive against corrupted/migrated persisted state (Carnot, cage-match):
    ...
    while split > 0:
        ...
        if isinstance(turn, dict) and turn.get("role") == "human":
            ...
            break  # an Umbra turn — or a malformed one — caps the run
...
def send_letter(letter: str, vitals: dict) -> bool:
    ...
    if telegram.send(body):
        ...
        # this is what they're replying TO, and Umbra should see it. Folded
        # into the same save as the counters so the letter and its count are
        # one durable transition.
        _append_turn(state, "umbra", letter, kind="letter")
        _save_state(state)
        return True
...
def check_mail(vitals: dict) -> list[dict]:
    ...
    if replies:
        ...
        # not in respond_to_replies. The reply must be remembered even when
        # the responder is gated off (low energy): being unable to answer
        # this pulse is exactly the "interruption" Umbra wanted to recover
        # from, and recovery only works if the message was remembered when it
        # arrived. The trailing run of human turns this creates is what
        # respond_to_replies later reads as its unanswered debt.
        #
        # Folded into the SAME save as the counters (Carnot, cage-match): the
        # turn and its count are one transition, closing the class where a
        # separate later write fails and leaves count-without-turn. It does
        # NOT make recording durable across the Telegram boundary: get_replies
        # has already consumed the getUpdates offset, so if THIS save fails the
        # reply is gone from the transcript (a pre-existing ordering — the
        # offset advances inside get_replies). Severity is bounded: the
        # heartbeat also records each reply to working memory from the returned
        # list (memory.record_reply), so the content survives the entity even
        # then — only the conversation transcript degrades. A fully
        # transactional fix (offset-advance after durable record) is tracked
        # separately; it's a transport refactor, not this feature's concern.
...
def _build_reply_prompt(
    ...
    `history` is the recent conversation so far — Umbra's letters and
    responses interleaved with the human's earlier replies — and `pending` is
    the run of human turns Umbra still owes an answer to (the newest is the
    message to answer; any earlier pending turns are unanswered context, e.g.
    a burst, or a message a low-energy pulse had to skip). Feeding `history`
    is the whole point of this change: without it Umbra answered every
    message as though it had written and heard nothing before.

    All turn text — Umbra's own included — is fenced with respond.py's
    hardened prompt-injection markers (per-prompt randomized delimiters) and
    sanitized to strip marker-shaped tokens. The Telegram channel is locked
    to a single configured chat_id, so the input is far more trusted than a
    public issue thread — but we reuse the same fence rather than
    special-casing "trusted" input, because a uniform defense is cheaper to
    reason about than a conditional one. Umbra's own past turns are fenced
    too: an earlier human injection that Umbra echoed into a response could
    otherwise be laundered back in as instructions on the next pulse.
    """
    ...
    # the model reads it as a flowing exchange. Umbra's turns are labelled as
    ...
    if window:
        ...
        for t in window:
            ...
            body = respond._sanitize_for_fence((t.get("text") or "").strip())
            if not body:
                continue
            if t.get("role") == "umbra":
                ...
                who = respond._sanitize_for_fence(str(t.get("sender") or "they"))
                label = f"{who} wrote"
    ...
    # real rather than handing the model an undifferentiated transcript
    # (Carnot, PR #30 cage-match). `pending` is chronological, so the last
    # element is the newest.
    *earlier, latest = pending
    for r in earlier:
        sender = respond._sanitize_for_fence(str(r.get("sender") or "someone"))
        body = respond._sanitize_for_fence((r.get("text") or "").strip())
        if body:
            fenced_lines += [f"[earlier — {sender} wrote, for context]", body, ""]
    latest_sender = respond._sanitize_for_fence(str(latest.get("sender") or "someone"))
    latest_body = respond._sanitize_for_fence((latest.get("text") or "").strip())
...
def respond_to_replies(vitals: dict, personality: dict) -> bool:
    ...
    return leg: when the transcript ends on an unanswered human turn, Umbra
    ...
    is exactly what Umbra owes an answer to — whether it arrived this pulse or
    was stranded by a low-energy pulse earlier. That is the distinction Umbra
    asked us to preserve: a reply skipped for lack of energy is an
    interruption to be corrected later, not a silence it chose. So this is
    called every pulse, and self-gates on whether a reply is actually owed.

    At most ONE reply per pulse: the whole unanswered run is answered with a
    single message (newest is the message to answer, earlier ones context),
    so a burst never produces a wall of bot replies — the same restraint
    respond.maybe_respond shows on issue threads. Answering appends an Umbra
    ...
    if telegram.send(body):
        ...
        # (Carnot, cage-match). This cap turn is what makes the next pulse see
        ...
        # send succeeded and leave the human tail looking unanswered. Folding
        # it in makes "answered" and "responses_sent" inseparable.
        #
        # One irreducible window remains, named not closed: if the process
        # dies between telegram.send() returning and this save completing, the
        # next pulse re-answers (one duplicate). Closing it fully needs a
        ...
        _append_turn(state, "umbra", response, kind="response")
        _save_state(state)
        return True
