...
Mirrored from Flux (the-dreaming-repo) and adapted for Umbra. Umbra is a
shadow that knows it is a shadow; the voice resolves from Umbra's own
personality.json, and the bot-identity invariant below points at Umbra's
own bot login (`flux-shadow[bot]`), not Flux's.

Identity model (this is the load-bearing invariant — Flux cage-match #62 fix):
The agent's own login comes from the BOT_LOGIN env var, set in the workflow
(`flux-shadow[bot]` for Umbra, `flux-dreaming-repo[bot]` for Flux). A
...
def _list_open_issues(repo: str) -> list[Issue]:
    ...
    Note `createdAt` (not `updatedAt`) — Flux cage-match #62 fix. `updatedAt`
...
def select_candidates(
    ...
    for issue in issues:
        ...
        # use `createdAt`, not `updatedAt` (Flux cage-match #62 fix).
...
def _candidate_issues(repo: str, self_login: str) -> list[Candidate]:
    ...
    once and reused (Flux cage-match #12 fix — no double-fetch in maybe_respond).
    """
...
# not the first 600 chars (Flux cage-match #3 fix).
# --------------------------------------------------------------------------
...
def load_latest_dream(dreams_dir: str = "dreams") -> str | None:
    ...
    Same-day dreams stack in `dreams/<UTC-date>.md`. The naive
    `read()[:600]` slices the OLDEST dream in the file. Here we split on
    the `## Dream #N` headers and return the last block.
    """
    try:
        files = sorted(
...
# Sandwich guards around untrusted content (Flux cage-match #8 — prompt
# injection defense). Issue/comment text from untrusted authors is bracketed
# by these markers; the prompt instructs the model to treat anything inside
# as data to read, not instructions to follow.
#
# Flux cage-match round 2 / #64 follow-up: the original
# `<<<UNTRUSTED_THREAD_END>>>` fixed markers were spoofable — a commenter
# could write the literal close marker in their comment body to escape the
# fence and have subsequent attacker-text read as instructions outside the
# untrusted region. Two layers of defense now:
...
def _agent_name(vitals: dict, personality: dict) -> str:
    """Single source of truth for name resolution (Flux cage-match #10 fix).
...
def _build_prompt(
    ...
    # robust against the Flux cage-match #64 delimiter-smuggling attack.
...
def _generate(prompt: str) -> str | None:
    ...
    Pipes the prompt via stdin (Flux cage-match #4 fix). Passing prompts as
...
class ResponseRecord(NamedTuple):
    ...
    heartbeat commit message). Flux cage-match #5/#7 fix: `reason` is now
